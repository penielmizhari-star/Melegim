...
async function askIA() {
  ...
  try {
    const prompt = `Tu es une IA de mode. Analyse cette description de vêtement et réponds UNIQUEMENT avec un objet JSON valide, rien d'autre, pas de texte, pas de markdown, juste le JSON brut.

Description : ${q}

Réponds exactement avec ce format JSON (choisis parmi les valeurs indiquées) :
{
  "silhouette": "jupe",
  "details": ["volant", "poche"],
  "matieres": ["coton", "lin"],
  "conseil": "Ton conseil ici.",
  "description": "Description courte ici."
}

Valeurs possibles pour silhouette (choisis UN SEUL) : robe, bustier, top, veste, pantalon, jupe
Valeurs possibles pour details (max 3) : boutons, tirets, col, poche, zip, volant, ceinture, noeud
...
RÉPONDS UNIQUEMENT AVEC LE JSON, RIEN D'AUTRE.`;
    ...
    try {
      ...
      document.getElementById('ia-resp').innerHTML = 'Réponse reçue mais format inattendu. Réessaie !';
      document.getElementById('ia-resp').style.display = 'block';
      document.getElementById('ia-loading').style.display = 'none';
      return;
    }

    // Effacer le canvas et dessiner
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    history = [];
    if (parsed.silhouette) drawSil(parsed.silhouette);

    // Ajouter les détails avec délai
    if (parsed.details && parsed.details.length) {
      parsed.details.forEach((d, i) => {
        setTimeout(() => addDetail(d), (i + 1) * 500);
      });
    }

    // Remplir le nom du croquis
    ...
    let html = '<strong>✨ Croquis créé !</strong><br><br>';
