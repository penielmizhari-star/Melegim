  ...
  try {
    const resp = await fetch('https://winter-sun-345c.peniel-mizhari.workers.dev', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: "Tu es une assistante créative en mode. La créatrice décrit un vêtement et tu dois répondre UNIQUEMENT en JSON valide, sans texte avant ou après, avec ce format exact : {"silhouette": "robe", "details": ["boutons", "col"], "matieres": ["lin", "coton"], "conseil": "Un petit conseil de style.", "description": "Description courte et enthousiaste du vêtement créé."}. Pour silhouette, choisis UN SEUL parmi : robe, bustier, top, veste, pantalon, jupe. Pour details, choisis parmi : boutons, tirets, col, poche, zip, volant, ceinture, noeud. Maximum 3 details.",
    ...
    try {
      ...
      document.getElementById('ia-resp').innerHTML = txt.replace(/\n/g, '<br>');
      document.getElementById('ia-resp').style.display = 'block';
      document.getElementById('ia-loading').style.display = 'none';
      return;
    }

    // Effacer le canvas et dessiner la silhouette
    if (parsed.silhouette) {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      history = [];
      drawSil(parsed.silhouette);
    }

    // Ajouter les détails avec un délai pour effet visuel
    if (parsed.details && parsed.details.length) {
      parsed.details.forEach((d, i) => {
        setTimeout(() => addDetail(d), (i + 1) * 400);
      });
    }
    ...
    if (parsed.description) {
      ...
      if (nameInput) nameInput.value = parsed.description.substring(0, 50);
    }

    // Ajouter les matières comme tags
    ...
    let html = '<strong>✨ Croquis généré !</strong><br><br>';
    if (parsed.description) html += parsed.description + '<br><br>';
    if (parsed.conseil) html += '<em>💡 ' + parsed.conseil + '</em>';
